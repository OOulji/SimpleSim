    ...
    def run(self) -> np.array:
        ...
        while sim_time < self._time_limit:
            ...
            for i, cb1 in enumerate(self._celestial_bodies):
                ...
                velocity = self._integrator.integrate(self._universe.time_step,sim_time, cb1.get_velocity(), acceleration)
                new_state[i].update_velocity(velocity)
                position = self._integrator.integrate(self._universe.time_step,sim_time, cb1.get_position(),velocity)
